...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
	    ...
	    $this->guid            = $this->config->item('guid', 'ion_auth');
	}
	 ...
	 * @author Ammon Casey
	 **/
	public function get_field($table = false, $id = false, $columns = false)
	{
		//if no id was passed use the current users id
		...
		if (!empty($columns))
		{
		    if (is_array($columns))
		    {
		        ...
		        {
		            $this->db->select($this->tables[$table].'.'.$col);
		        }
		    }
		    else
		    {
		        $this->db->select($this->tables[$table].'.'.$columns);
		    }
		}
		else
		{
		    $this->db->select($this->tables[$table].'.*');
		}
		                       
		$this->db->where($this->tables[$table].'.id', $id);
		$this->db->limit(1);

		return $this->db->get($this->tables[$table]);
	}
	 ...
	 * @author Ammon Casey
	 **/
	public function log_changes($data)
	{
	    if (empty($data))
	    {
	        return FALSE;
	 ...
	 * @author Ammon Casey
	 **/
	 public function count_records($table = false, $key = false, $value = false)
	 {
	     if ( empty($table) ) {
	         return FALSE;
	     }
